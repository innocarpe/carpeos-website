...
- Immutable source commit (release tag `v5.0.0`): `bba6fdbdd19155aea2d44efacde742afe79a7c36`
...
Line ranges below refer to the immutable source documents at `v5.0.0` unless noted. Each row covers one rendered factual sentence, status, command, or destination family. Repeated navigation links use the same approved destination row.
...
| `principle-adjudication` | “Adjudicated meaning” | implemented model / local-only | `README.md` Adjudicate before “memory”; `CHANGELOG.md` 2.0/3.2 | `adj_v3` remains foundation under 5.0 | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `how-capture` | Selected lifecycle events enter an encrypted outbox and local store; host hooks stay fail-open and fast. | local-only | `README.md` Capture from tools | Local capture and outbox are implemented; V5 LLM path is not on the capture hot path | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `interface-cli` | CLI supports capture, adjudication, search, rebuild, and context packs. | local-only | `README.md` CLI bullet list; `CHANGELOG.md` 5.0.0 | Published CLI `5.0.0` retains `adjudicate`, `memory`, `okf` and adds opt-in `carpeos v5` surfaces | `innocarpe` · 2026-08-06 · `APPROVED` |
| `interface-mcp` | MCP over stdio exposes nine local tools, including `memory_neighborhood`. | local-only | `docs/contracts/mcp-tools-v1.json` (9 tools) | Runtime inventory is nine tools at `v5.0.0` | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `interface-okf` | OKF v0.2 export writes a trust-zone-scoped, promoted-by-default portable bundle; export only, not import or canonical storage. | local-only / projection | `README.md` OKF bullet; `docs/guides/okf-export.md`; `CHANGELOG.md` 3.1.0 | Shipped in 3.1; retained in 5.0 package | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `model-append-only` | Dispositions and events are append-only by policy version. | implemented model | `README.md` How it fits rules; `CHANGELOG.md` 2.0/3.2/5.0 | Model semantics only; V5 draft records stay `canonical_effect: "none"` | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `install-node` | CarpeOS requires Node.js 22.22 or newer. | current prerequisite | `README.md` Install; npm engines `>=22.22.0` | npm `@innocarpe/carpeos@5.0.0` reports `engines.node >=22.22.0` | `innocarpe` · 2026-08-06 · `APPROVED` |
| `install-npm` | `npm install -g @innocarpe/carpeos` | release-verified command | `README.md` Install; product 3.2 activation path retained | `@innocarpe/carpeos@5.0.0` is published on npm | `innocarpe` · 2026-08-06 · `APPROVED` |
| `install-pin` | Pin `@innocarpe/carpeos@5.0.0` for a fixed release. | release pin guidance | npm registry exact version; GitHub Release `v5.0.0` | Exact version exists on registry | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `install-apply` | `carpeos setup run --apply` | release-verified mutating command | `README.md` setup; product 3.2 activation | Last formal disposable-home activation receipt remains product 3.2; setup path is still the documented product path in 5.0 | `innocarpe` · 2026-08-06 · `APPROVED_WITH_CAVEAT` |
...
| `install-hosting-boundary` | “Local-first. Interfaces ship on your machine; no hosted Worker, hosted MCP, or SaaS deployment is claimed.” | local-only / not deployed | `README.md` What works today NOT DEPLOYED; product 5.0 DoD non-goals | No hosted capability is presented as live | `innocarpe` · 2026-08-06 · `APPROVED` |
...
| `footer-version` | Footer release link → `https://github.com/innocarpe/carpeos/releases/tag/v5.0.0` | current release destination | GitHub Release `v5.0.0`; npm `5.0.0` | Non-draft, non-prerelease release exists | `innocarpe` · 2026-08-06 · `APPROVED` |
...
|---|---|---|---|---|---|
| `docs-status` | CarpeOS is local-first and open source; current public package is `@innocarpe/carpeos@5.0.0`; contracts, ADRs, product DoDs, and verification live in the public source. | shipped release / local-only | `CHANGELOG.md` `[5.0.0]`; npm `@innocarpe/carpeos@5.0.0`; GitHub Release `v5.0.0` | Public destinations are versioned with source | `innocarpe` · 2026-08-06 · `APPROVED` |
...
Verified on 2026-08-06 against product `5.0.0`:

- Immutable GitHub Release: <https://github.com/innocarpe/carpeos/releases/tag/v5.0.0> (`v5.0.0`, not a draft or prerelease).
- Version-specific npm metadata: <https://registry.npmjs.org/@innocarpe%2Fcarpeos/5.0.0> (`@innocarpe/carpeos@5.0.0`, Node.js `>=22.22.0`).
